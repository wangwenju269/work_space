...
class React(GET_TOOL_DESC):
    ...
    def construct_react_prompt(self, query, history, **kwargs):
        ...
        self.collect_user_data[self.unique_id].extend([{'from':"<|user|>", 'value':f'<|im_start|>user\n{prompt[len(self.systems) + 1 :]}<|im_end|>\n<|im_start|>assistant\n'}])
        return prompt 
    ...
    def infer(self, planning_prompt, tool_name):
        ...
        while count <= 8:
            ...
            self.collect_user_data[self.unique_id].extend([{'from':"<|assistant|>", 'value':f'{response}'}])
            ...
            if  (action in tool_name) and action :  # 需要调用插件
                ...
                self.collect_user_data[self.unique_id].extend([{'from':"<|Observation|>",'value': f'{api_output}<|im_end|>\n<|im_start|>assistant\n'}])
