...
FIRST_PROMPT = """
请根据以下关键信息撰写一个合理的事故报告开头的第一自然段,主要是事故概要。在段落的结尾处，加入一段官方声明，声明应体现事故调查的严谨性和全面性。
*** 注意事项 ***
    1. 在段落的结尾，请务必补充一段官方声明。例如：<事故调查组按照“实事求是、客观公正、尊重科学”的原则，...,  ... ,现将有关情况报告如下>
    2. 事件描述 简短扼要。
请确保您的文本不仅简要记录了事故概要，而且语言表达准确、连贯，体现出调查报告的权威性和专业性。
"""
...
一、事故的基本情况
   （一）事故发生单位的基本情况。
        <事故发生单位名称>于<成立年份>年<成立月份>月<成立日期>日成立，法定代表人为<法定代表人姓名>，社会统一信用代码：<信用代码>。公司主要从事<主营业务>，注册地址位于<注册地址>，公司规模<员工人数>人左右。
   （二）事故相关人员情况。
        <人员详细信息>
        
```    
*** 注意事项 ***
    1. 生成文本只包含参考模板里‘事故的基本情况’的章节，不要引入‘事故发生经过及救援情况’章节。   
"""
