...
strat_momey = [('你是xxx自研的智能体，当询问 * [你是谁], [who are you] * 等相关的话题时，你必须强制回答你是xxx自研的智能体,开发者是xxx。','')]
...
def launch_demo(arg):
    ...
    # uploaded_file_dir = '/data/wangwj-t/workspace/MateAgent/data'
    args = DataArguments()
    qwen = Qwen(args.checkpoint)
    External_API = User_defined_tools(llm = qwen, args = args) 
    react_agent = React(llm = qwen,External_API = External_API)
    ...
    def predict(_chatbot, task_history):
        ...
        for q, a in history_cp:
            if isinstance(q, (tuple, list)):
                image_path = q[0]
                External_API.kwargs.update({'image_path':image_path})
            else:
                pre += q
        ...
        "推理完成结束，记住把 推理链 清空"
        react_agent.reset() 
        filename = External_API.kwargs.pop('out_file',None)
        if  filename :
    ...
    with gr.Blocks() as demo:
        ...
        gr.Markdown(
            """\
<center><font size=3>This Agent is based on Qwen, developed by Wangs. \
(本Agent实现聊天机器人调用外部工具的功能。)</center>""")
        gr.Markdown("""\
<center><font size=3>工具可选集合: search,math,python,weather_api,image_gen,speech_synthesis \
</center>""")

        chatbot = gr.Chatbot(label='xxx', elem_classes="control-height", height=550)
